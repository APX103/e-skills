...
metadata:
  hermes:
    category: research
    tags: [research, knowledge-production, experiments, evidence]
---
...
Do not use for one-off explanations, direct coding tasks, simple bug fixes, or routine code review. Use `e-build` for implementation-only work and `e-think` for standalone reflection on an existing result.
